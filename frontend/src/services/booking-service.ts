...
export const bookingService = {
  ...
  getMockBookingById(id: string): Booking {
    ...
    // First try to get from mock bookings in localStorage
    const storedBookings = this.getAllStoredMockBookings();
    const booking = storedBookings.find((b) => b._id === id);

    if (booking) {
      return booking;
    }

    // Check individual booking (legacy approach)
  ...
  getMockTicketData(bookingId: string): TicketData {
    ...
    // Check in overall mock bookings first (new approach)
    const storedBookings = this.getAllStoredMockBookings();
    booking = storedBookings.find((b) => b._id === bookingId) || null;
    ...
    const passengers = booking
      ? booking.passengers.map((p, i) => {
          ...
          // Add null check for booking when accessing seatNumbers
          const seatNumber =
  ...
  /**
   * Get all stored mock bookings from localStorage
   */
  getAllStoredMockBookings(): Booking[] {
    try {
      const bookingsJson = localStorage.getItem(MOCK_BOOKINGS_KEY);
      if (bookingsJson) {
        return JSON.parse(bookingsJson);
      }

      // If no central storage found, attempt to migrate legacy storage
      const legacyBookings: Booking[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && key.startsWith("booking_")) {
          try {
            const booking = JSON.parse(localStorage.getItem(key) || "");
            legacyBookings.push(booking);
          } catch (e) {
  ...
  saveMockBookingToStorage(booking: Booking): void {
    try {
      ...
      const existingBookings = this.getAllStoredMockBookings();
      ...
      localStorage.setItem(MOCK_BOOKINGS_KEY, JSON.stringify(existingBookings));
