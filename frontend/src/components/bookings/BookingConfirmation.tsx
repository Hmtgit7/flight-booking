    ...
    useEffect(() => {
        const fetchBooking = async () => {
            try {
                ...
                // Check if this is a mock booking ID (non-MongoDB format)
                const isMockBookingId = id.startsWith('booking_');

                if (isMockBookingId) {
                    // Generate mock booking data instead of fetching from backend
                    console.log("Using mock booking data for ID:", id);
                    const mockBooking = await bookingService.getMockBookingById(id);
                    setBooking(mockBooking);

                    // Generate mock ticket data
                    const mockTicket = await bookingService.getMockTicketData(id);
                    setTicket(mockTicket);
                } else {
                    // This is a real MongoDB ID, fetch from backend
